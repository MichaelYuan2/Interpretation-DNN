...
def set_inplace_to_false(model):
    for module in model.modules():
        if isinstance(module, nn.ReLU) or isinstance(module, nn.PReLU) or isinstance(module, nn.LeakyReLU):
...
def replace_relu_with_newins(model):
    for name, module in model.named_children():
        if isinstance(module, nn.ReLU):
            setattr(model, name, nn.ReLU())
        elif len(list(module.children())) > 0:
            replace_relu_with_newins(module)
    return model 
# Example usage:
...
try:
    ...
    resnet50 = set_inplace_to_false(resnet50)
    resnet50 = replace_relu_with_newins( resnet50 )
